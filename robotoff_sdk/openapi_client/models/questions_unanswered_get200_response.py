...
class QuestionsUnansweredGet200Response(BaseModel):
    """
    QuestionsUnansweredGet200Response
    """ # noqa: E501
    count: StrictInt = Field(description="The total number of questions that meet the provided criteria")
    questions: List[QuestionsUnansweredGet200ResponseQuestionsInner]
    status: StrictStr = Field(description="The request status")
    __properties: ClassVar[List[str]] = ["count", "questions", "status"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['found', 'no_questions']):
            raise ValueError("must be one of enum values ('found', 'no_questions')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "count": obj.get("count"),
            "questions": [QuestionsUnansweredGet200ResponseQuestionsInner.from_dict(_item) for _item in obj["questions"]] if obj.get("questions") is not None else None,
